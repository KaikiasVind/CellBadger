...
/**
 * @brief findClusterCellFoldChangeCorrelations - Correlates the given cluster to EVERY given tissue - Runs in O(n*m) at least
 ...
 */
QVector<QPair<QString, double>> findClusterCellTypeFoldChangeCorrelations(FeatureCollection cluster, QVector<FeatureCollection> cellTypes) {

    // Create a list that will contain the correlation of all clusters with all cell types
    QVector<QPair<QString, double>> cellTypeCorrelationsForAllClusters;

    for (int i = 0; i < cellTypes.length(); i++) {
        QVector<double> clusterFeatureFoldChanges, cellTypeFeatureFoldChanges;

        for (int j = 0; j < cellTypes[i].getNumberOfFeatures(); j++) {

            double cellTypeFeatureFoldChange = cellTypes[i].getFeatureFoldChange(j);
            ...
            for (int k = 0; k < cluster.getNumberOfFeatures(); k++) {
                ...
                bool isFeatureIDEqual = cellTypes[i].getFeatureID(j).compare(cluster.getFeatureID(k)) == 0;

                if (isFeatureIDEqual) {
                    isFeatureExpressedInCluster = true;

                    clusterFeatureFoldChanges.append(cluster.getFeatureFoldChange(k));
        ...
        cellTypeCorrelationsForAllClusters.append(qMakePair(cellTypes[i].ID, correlation));
    }
    ...
    std::sort(cellTypeCorrelationsForAllClusters.begin(), cellTypeCorrelationsForAllClusters.end(),
...
/**
 * @brief findClusterCellFoldChangeCorrelations - Correlates EVERY given cluster to EVERY given tissue - Runs in O(n*m) at least
 * @param clusters - List of clusters (CellRanger t-SNE clusters)
 * @param tissues - List of tissues (One tissue contains cell markers with corresponding expression counts - see BioModels/FeatureCollection)
 * @return - Sorted correlations of every cluster and every tissue
 */
QVector<QVector<QPair<QString, double>>> findCellFoldChangeCorrelationsForAllClusters(QVector<FeatureCollection> clusters, QVector<FeatureCollection> cellTypes) {
...
//                  [](QPair<QString, double> pairA, QPair<QString, double> pairB) { return pairA.second > pairB.second; });
        ...
        cellTypeCorrelationsForAllClusters.append(ExpressionComparator::findClusterCellTypeFoldChangeCorrelations(clusters.at(i), cellTypes));
    }
